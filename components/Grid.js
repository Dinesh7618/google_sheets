...
const Grid = () => {
  ...
  const handleCellChange = (row, col, text) => {
    ...
    saveGridData(updatedGridData); 
  };

  const handleDownload = async () => {
    const csvData = gridData.map((row) => row.join(',')).join('\n');
    const filePath = `${FileSystem.documentDirectory}/grid_data.csv`;
    ...
    try {
      ...
      Alert.alert('CSV File Created', 'The CSV file has been successfully created.');
